...
class MySQLAdapter {
    ...
    async query(sql, params) {
        const { translatedSql, translatedParams, returningColumns, tableName, injectedId, updateWhereIdIndex } = this.translateQuery(sql, params);
        try {
    ...
    async connect() {
        ...
        return {
            async query(sql, params) {
                const { translatedSql, translatedParams, returningColumns, tableName, injectedId, updateWhereIdIndex } = self.translateQuery(sql, params);
                if (returningColumns && tableName) {
                    ...
                    if (selectId) {
                        const selectSql = returningColumns === '*'
                            ? `SELECT * FROM ${tableName} WHERE id = ?`
                            : `SELECT ${returningColumns} FROM ${tableName} WHERE id = ?`;
                        const [rows] = await connection.execute(selectSql, [selectId]);
                        return { rows: rows, rowCount: rows.length };
                    }
                    return { rows: [], rowCount: result.affectedRows || 0 };
                }
     ...
     */
    translateQuery(sql, params) {
        let translatedSql = sql;
        let returningColumns = null;
        let tableName = null;
        let injectedId = null;
        let updateWhereIdIndex = null;
        ...
        if (insertHeaderMatch) {
            ...
            if (!hasIdColumn && !tablesWithoutId.includes(currentTable)) {
                injectedId = (0, crypto_1.randomUUID)();
                const fullMatch = insertHeaderMatch[0];
                const newPrefix = fullMatch.replace(`(${colListStr})`, `(id, ${colListStr})`) + '?, ';
        ...
        translatedSql = translatedSql.replace(/EXTRACT\s*\(\s*(MONTH|DAY|YEAR|HOUR|MINUTE|SECOND)\s+FROM\s+([^)]+)\)/gi, (_match, part, field) => `${part.toUpperCase()}(${field.trim()})`);
        // 7. INTERVAL 'N days/months/etc' → INTERVAL N DAY/MONTH/etc
        translatedSql = translatedSql.replace(/INTERVAL\s+'(\d+)\s+(days?|months?|years?|hours?|minutes?|seconds?)'/gi, (_match, num, unit) => {
        ...
        if (translatedSql.includes('uuid_generate_v4()')) {
            ...
            const extraUuid = (0, crypto_1.randomUUID)();
            if (translatedParams) {
        ...
        translatedSql = translatedSql.replace(/ON\s+CONFLICT\s*\([^)]+\)\s+DO\s+UPDATE\s+SET\s+(.+?)(?=;|\s*$)/gi, (_match, setClauses) => {
        ...
        translatedSql = translatedSql.replace(/date_trunc\s*\(\s*'(\w+)'\s*,\s*([^)]+)\)/gi, (_match, precision, field) => {
        ...
        translatedSql = translatedSql.replace(/to_char\s*\(\s*([^,]+),\s*'([^']+)'\)/gi, (_match, field, pgFormat) => {
        ...
        translatedSql = translatedSql.replace(/(\w+)\s*=\s*ANY\s*\(\s*\?\s*\)/gi, 'FIND_IN_SET($1, ?)');
        return {
            translatedSql,
            translatedParams,
            returningColumns,
