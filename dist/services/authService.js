...
class AuthService {
    ...
    async login(email, password) {
        ...
        try {
            ...
            const result = await this.db.query('SELECT id, name, email, password_hash, role, allowed_modules_json, avatar_initials, active, created_at FROM system_users WHERE LOWER(email) = ? AND active = 1', [cleanEmail]);
            if (result.rows.length > 0) {
                const dbUser = result.rows[0];
                const passwordMatch = await bcrypt_1.default.compare(cleanPass, dbUser.password_hash);
                if (passwordMatch) {
