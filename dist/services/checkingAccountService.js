...
class CheckingAccountService {
     ...
     */
    async addMovement(customerId, movementType, amount, referenceType, description, referenceId, clientOverride) {
        ...
        try {
            ...
            const custRes = await client.query('SELECT id, first_name, last_name, credit_limit, current_account_balance FROM customers WHERE id = $1 FOR UPDATE', [customerId]);
      ...
      `;
            ...
            await client.query('UPDATE customers SET current_account_balance = $1, updated_at = CURRENT_TIMESTAMP WHERE id = $2', [newBalance, customerId]);
            if (isLocalTransaction) {
    ...
    async updateCreditLimit(customerId, creditLimit) {
        try {
            await this.db.query('UPDATE customers SET credit_limit = $1, updated_at = CURRENT_TIMESTAMP WHERE id = $2', [creditLimit, customerId]);
        }
        catch {
            // Fallback
        }
     ...
     */
    async registrarCobroCliente(dto) {
        ...
        try {
            ...
            try {
                ...
                const lastRecRes = await client.query(`SELECT receipt_number FROM payments WHERE receipt_number LIKE $1 ORDER BY receipt_number DESC LIMIT 1`, [`${prefijoRecibo}%`]);
        ...
        `;
                ...
                const metodoDesc = dto.metodoPago === 'CASH'
                    ? 'Efectivo'
                    : dto.metodoPago === 'MERCADO_PAGO'
                        ? 'Mercado Pago'
                        : 'Transferencia';
                const desc = `Cobro a Cuenta Recibo ${numeroRecibo} (${metodoDesc})${dto.numeroComprobanteRef ? ' Ref: ' + dto.numeroComprobanteRef : ''}`;
                const movimiento = await this.addMovement(dto.clienteId, 'CREDIT', dto.monto, 'PAYMENT', desc, paymentId, client);
                // 4. Si el cobro se asocia a un pedido específico, actualizar saldo del pedido
                let numeroPedido;
                if (dto.pedidoId) {
                    const ordRes = await client.query('SELECT order_number, total_amount, paid_amount FROM orders WHERE id = $1 FOR UPDATE', [dto.pedidoId]);
                    if (ordRes.rows.length > 0) {
                        ...
                        await client.query('UPDATE orders SET paid_amount = $1, balance_due = $2, payment_status = $3, updated_at = CURRENT_TIMESTAMP WHERE id = $4', [nuevoPaid, nuevoBalanceDue, nuevoStatus, dto.pedidoId]);
                    }
                }
                await client.query('COMMIT');
     ...
     */
    async getExtractoDetallado(filtro) {
        ...
        try {
            // 1. Datos cliente
            const custRes = await this.db.query('SELECT id, first_name, last_name, phone_whatsapp, email, address, credit_limit, current_account_balance FROM customers WHERE id = $1', [clienteId]);
            if (custRes.rows.length > 0) {
           ...
           ORDER BY created_at ASC`, [clienteId, `${periodStart} 00:00:00`, `${periodEnd} 23:59:59`]);
                ...
                for (const row of movRes.rows) {
                    ...
                    if (incluirDetalleItems && row.reference_type === 'ORDER' && row.reference_id) {
                        try {
                            const orderRes = await this.db.query('SELECT order_number FROM orders WHERE id = $1', [row.reference_id]);
                        ...
                        try {
                            const payRes = await this.db.query('SELECT receipt_number, payment_method FROM payments WHERE id = $1', [row.reference_id]);
                ...
                const resumenWhatsappFormateado = this.generateWhatsappDetailedStatementText(customerName, periodStart, periodEnd, initialBalance, totalDebitos, totalCreditos, saldoFinalDeudor, movimientosDetallados);
                return {
                    clienteId,
     ...
     */
    generateWhatsappDetailedStatementText(nombreCliente, fechaInicio, fechaFin, saldoInicial, totalDebitos, totalCreditos, saldoFinal, movimientos) {
    ...
    addMovementInMemory(customerId, movementType, amount, referenceType, description, referenceId) {
    ...
    registrarCobroClienteInMemory(dto, summaryAnterior, numeroRecibo) {
        ...
        this.addMovementInMemory(dto.clienteId, 'CREDIT', dto.monto, 'PAYMENT', `Cobro a Cuenta Recibo ${numeroRecibo} (${dto.metodoPago})`, paymentId);
        this.inMemoryCollections.unshift(recibo);
        return recibo;
    }
    getExtractoDetalladoInMemory(filtro, summary, periodStart, periodEnd) {
        const clientMovs = this.inMemoryMovements.filter(m => m.customerId === filtro.clienteId);
        let totalDebitos = 0;
        let totalCreditos = 0;
        const movimientosDetallados = clientMovs.map(m => {
            if (m.movementType === 'DEBIT')
                totalDebitos += m.amount;
            else
                totalCreditos += m.amount;
            return {
                id: m.id,
        ...
        const resumenWhatsapp = this.generateWhatsappDetailedStatementText(summary.customerName, periodStart, periodEnd, 0, totalDebitos, totalCreditos, saldoFinal, movimientosDetallados);
        return {
