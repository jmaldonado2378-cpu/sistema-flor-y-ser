...
class PaymentService {
    ...
    constructor(db, checkingAccountService) {
        this.db = db;
    ...
    async generateReceiptNumber() {
        ...
        const res = await this.db.query(`SELECT receipt_number FROM payments WHERE receipt_number LIKE $1 ORDER BY receipt_number DESC LIMIT 1`, [`${prefix}%`]);
    ...
    async registerPayment(dto) {
      ...
      `;
            ...
            if (dto.orderId) {
                const orderRes = await client.query('SELECT total_amount, paid_amount FROM orders WHERE id = $1 FOR UPDATE', [dto.orderId]);
             ...
             WHERE id = $4`, [newPaidAmount, newBalanceDue, newPaymentStatus, dto.orderId]);
                }
            }
            ...
            if (dto.paymentMethod !== 'CURRENT_ACCOUNT_CREDIT') {
                const methodDesc = dto.paymentMethod === 'CASH'
                    ? 'Efectivo'
                ...
                await this.checkingAccountService.addMovement(dto.customerId, 'CREDIT', dto.amount, 'PAYMENT', desc, row.id, client);
            }
            else if (dto.orderId) {
                // Venta a Cuenta Corriente: registra DÉBITO (aumenta la deuda del cliente)
                const desc = `Compra a Cta Cte Recibo ${receiptNumber}`;
                await this.checkingAccountService.addMovement(dto.customerId, 'DEBIT', dto.amount, 'ORDER', desc, dto.orderId, client);
            }
            await client.query('COMMIT');
            // 4. Obtener nombre del cliente y número de orden
            const customerRes = await this.db.query('SELECT first_name, last_name FROM customers WHERE id = $1', [dto.customerId]);
