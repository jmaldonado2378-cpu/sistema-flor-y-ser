...
class MarketingService {
    ...
    constructor(db, customerService) {
        this.db = db;
        this.customerService = customerService || new customerService_1.CustomerService(db);
    }
    // =========================================================================
     ...
     */
    async getAudienceTargets(filter) {
        ...
        return customers.filter(customer => {
            ...
            if (filter.dietaryProfileIds && filter.dietaryProfileIds.length > 0) {
                const hasMatchingDiet = customer.dietaryProfiles.some(dp => filter.dietaryProfileIds.includes(dp.id) || filter.dietaryProfileIds.includes(dp.code));
                if (!hasMatchingDiet) {
                    return false;
                }
            }
