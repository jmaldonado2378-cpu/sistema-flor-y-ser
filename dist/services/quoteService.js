...
class QuoteService {
    ...
    async generateQuoteNumber() {
        ...
        const res = await this.db.query(`SELECT quote_number FROM quotes WHERE quote_number LIKE $1 ORDER BY quote_number DESC LIMIT 1`, [`${prefix}%`]);
    ...
    async createQuote(dto) {
      ...
      `;
            ...
            for (const item of processedItems) {
                await client.query(`INSERT INTO quote_items (
    ...
    `;
        ...
        const itemsRes = await this.db.query(`SELECT id, quote_id, product_name, product_id, is_bulk_fractioned, quantity, unit_price, subtotal, notes
       FROM quote_items WHERE quote_id = $1`, [id]);
        const items = itemsRes.rows.map((iRow) => ({
            id: iRow.id,
    ...
    async getQuotes(filter = {}) {
        ...
        if (search) {
            conditions.push(`(q.quote_number ILIKE $${paramIdx} OR c.first_name ILIKE $${paramIdx} OR c.last_name ILIKE $${paramIdx})`);
            params.push(`%${search}%`);
            paramIdx++;
        }
        const whereClause = conditions.length > 0 ? `WHERE ${conditions.join(' AND ')}` : '';
        const countRes = await this.db.query(`SELECT COUNT(*) as total FROM quotes q JOIN customers c ON c.id = q.customer_id ${whereClause}`, params);
    ...
    `;
        ...
        const quotes = await Promise.all(res.rows.map(async (row) => {
            const itemsRes = await this.db.query(`SELECT id, quote_id, product_name, product_id, is_bulk_fractioned, quantity, unit_price, subtotal, notes
           FROM quote_items WHERE quote_id = $1`, [row.id]);
            const items = itemsRes.rows.map((iRow) => ({
                id: iRow.id,
                quoteId: iRow.quote_id,
                productName: iRow.product_name,
     ...
     */
    async convertQuoteToOrder(quoteId, dto = {}) {
        ...
        const initialPayment = dto.initialPaymentMethod && dto.initialPaymentAmount
            ? {
