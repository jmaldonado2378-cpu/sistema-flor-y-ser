...
exports.FidelizationService = void 0;
const fidelization_1 = require("../types/fidelization");
class FidelizationService {
    db;
    customerService;
    inMemoryHistory = [
        {
            ...
            points: 85,
            transactionType: fidelization_1.PointTransactionType.ACCUMULATION,
        ...
        },
        {
            ...
            customerId: 'c1000000-0000-0000-0000-000000000001',
            points: 1042,
            transactionType: fidelization_1.PointTransactionType.ADJUSTMENT,
     ...
     */
    calculatePointsForAmount(amountSpent) {
        if (amountSpent <= 0)
            return 0;
    ...
    async accumulatePoints(dto) {
        ...
        try {
            ...
            const updateRes = await client.query(`UPDATE customers SET points_balance = points_balance + $1, updated_at = CURRENT_TIMESTAMP WHERE id = $2 RETURNING points_balance;`, [pointsEarned, dto.customerId]);
            const newBalance = updateRes.rows[0].points_balance;
            await client.query(`INSERT INTO customer_points_history (customer_id, points, transaction_type, reference_type, reference_id, description)
         VALUES ($1, $2, 'ACCUMULATION', 'ORDER', $3, $4);`, [dto.customerId, pointsEarned, dto.referenceId || null, description]);
    ...
    async accumulateInMemory(customerId, pointsEarned, description, referenceId) {
    ...
    async redeemPoints(dto) {
        ...
        try {
            await client.query('BEGIN');
            const updateRes = await client.query(`UPDATE customers SET points_balance = points_balance - $1, updated_at = CURRENT_TIMESTAMP WHERE id = $2 RETURNING points_balance;`, [dto.pointsToRedeem, dto.customerId]);
            const newBalance = updateRes.rows[0].points_balance;
            await client.query(`INSERT INTO customer_points_history (customer_id, points, transaction_type, reference_type, reference_id, description)
         VALUES ($1, $2, 'REDEMPTION', 'DISCOUNT', $3, $4);`, [dto.customerId, -dto.pointsToRedeem, dto.referenceId || null, description]);
            await client.query('COMMIT');
    ...
    async redeemInMemory(customerId, pointsToRedeem, discountAmount, description, referenceId) {
        const customer = await this.customerService.getUnifiedProfile(customerId);
        ...
        this.inMemoryHistory.unshift({
            ...
            transactionType: fidelization_1.PointTransactionType.REDEMPTION,
    ...
    async adjustPoints(dto) {
        ...
        try {
            await client.query('BEGIN');
            const updateRes = await client.query(`UPDATE customers SET points_balance = GREATEST(0, points_balance + $1), updated_at = CURRENT_TIMESTAMP WHERE id = $2 RETURNING points_balance;`, [dto.pointsDelta, dto.customerId]);
            const newBalance = updateRes.rows[0].points_balance;
            await client.query(`INSERT INTO customer_points_history (customer_id, points, transaction_type, description)
         VALUES ($1, $2, 'ADJUSTMENT', $3);`, [dto.customerId, dto.pointsDelta, description]);
            await client.query('COMMIT');
            return { newBalance };
        }
        catch {
    ...
    async getPointsSummary(customerId) {
        ...
        history.forEach(h => {
            if (h.points > 0)
                totalAccumulated += h.points;
            if (h.points < 0)
                totalRedeemed += Math.abs(h.points);
        });
        return {
            customerId,
...
exports.FidelizationService = FidelizationService;
