...
class SaleService {
    ...
    async generateOrderNumber() {
        ...
        const res = await this.db.query(`SELECT order_number FROM orders WHERE order_number LIKE $1 ORDER BY order_number DESC LIMIT 1`, [`${prefix}%`]);
    ...
    async createOrder(dto) {
      ...
      `;
          ...
          RETURNING id, order_id, product_name, product_id, is_bulk_fractioned, quantity, unit_price, subtotal, notes`, [
            ...
            if (pointsEarned > 0) {
                await client.query('UPDATE customers SET points_balance = points_balance + $1, updated_at = CURRENT_TIMESTAMP WHERE id = $2', [pointsEarned, dto.customerId]);
                await client.query(`INSERT INTO customer_points_history (
            customer_id, points, transaction_type, reference_type, reference_id, description
          ) VALUES ($1, $2, 'ACCUMULATION', 'ORDER', $3, $4)`, [dto.customerId, pointsEarned, orderId, `Puntos acumulados por Pedido ${orderNumber}`]);
            }
    ...
    `;
        ...
        // Obtener items
        const itemsRes = await this.db.query(`SELECT id, order_id, product_name, product_id, is_bulk_fractioned, quantity, unit_price, subtotal, notes
    ...
    async getOrders(filter = {}) {
        ...
        if (search) {
            conditions.push(`(o.order_number ILIKE $${paramIdx} OR c.first_name ILIKE $${paramIdx} OR c.last_name ILIKE $${paramIdx})`);
            params.push(`%${search}%`);
            paramIdx++;
        }
    ...
    `;
        ...
        const orders = await Promise.all(ordersRes.rows.map(async (row) => {
            const itemsRes = await this.db.query(`SELECT id, order_id, product_name, product_id, is_bulk_fractioned, quantity, unit_price, subtotal, notes
           FROM order_items WHERE order_id = $1`, [row.id]);
            const items = itemsRes.rows.map((iRow) => ({
                id: iRow.id,
                orderId: iRow.order_id,
                productName: iRow.product_name,
