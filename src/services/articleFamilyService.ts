import { ArticleFamily, CreateArticleFamilyDTO, UpdateArticleFamilyDTO } from '../types/articleFamily';

const initialFamilies: ArticleFamily[] = [
  // --- RAW MATERIALS ---
...
export class ArticleFamilyService {
  private inMemoryFamilies: ArticleFamily[] = [...initialFamilies];
  ...
  private async ensureTableExists(): Promise<void> {
    ...
    try {
      ...
      try { await this.db.query(`ALTER TABLE raw_materials ADD COLUMN family_id VARCHAR(64);`); } catch {}
      try { await this.db.query(`ALTER TABLE final_products ADD COLUMN family_id VARCHAR(64);`); } catch {}
      try { await this.db.query(`ALTER TABLE packaging_materials ADD COLUMN family_id VARCHAR(64);`); } catch {}
  ...
  private mapRowToFamily(row: any): ArticleFamily {
    return {
      id: row.id,
  ...
  async getAll(): Promise<ArticleFamily[]> {
  ...
  async getByScope(scope: string): Promise<ArticleFamily[]> {
    await this.ensureTableExists();
    try {
      const res = await this.db.query(`
        SELECT f.id, f.parent_id AS "parentId", f.code, f.name, f.description, 
  ...
  async getById(id: string): Promise<ArticleFamily | null> {
    await this.ensureTableExists();
    try {
      const res = await this.db.query(`
        SELECT f.id, f.parent_id AS "parentId", f.code, f.name, f.description, 
      ...
      `, [id]);
      if (res.rows.length > 0) return this.mapRowToFamily(res.rows[0]);
    } catch {}
  ...
  async create(dto: CreateArticleFamilyDTO): Promise<ArticleFamily> {
    await this.ensureTableExists();
    try {
      const res = await this.db.query(`
  ...
  async update(id: string, dto: UpdateArticleFamilyDTO): Promise<ArticleFamily | null> {
    await this.ensureTableExists();
    try {
      ...
      if (dto.parentId !== undefined) { updates.push(`parent_id = $${i++}`); values.push(dto.parentId); }
      ...
      if (dto.description !== undefined) { updates.push(`description = $${i++}`); values.push(dto.description); }
      if (dto.articleScope !== undefined) { updates.push(`article_scope = $${i++}`); values.push(dto.articleScope); }
      if (dto.icon !== undefined) { updates.push(`icon = $${i++}`); values.push(dto.icon); }
      if (dto.sortOrder !== undefined) { updates.push(`sort_order = $${i++}`); values.push(dto.sortOrder); }
