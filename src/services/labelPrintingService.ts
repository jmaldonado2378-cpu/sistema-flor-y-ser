import { 
  PrinterProtocol, 
  ProductThermalLabelDTO, 
  ShippingLogisticsLabelDTO, 
  LabelPrintResponseDTO 
} from '../types/printer';
...
export class LabelPrintingService {
   ...
   */
  generateTSPLProductLabel(dto: ProductThermalLabelDTO): string {
   ...
   */
  generateTSPLShippingLabel(dto: ShippingLogisticsLabelDTO): string {
    const width = dto.widthMm || 50;
   ...
   */
  generateNiimbotPacketData(commandType: number, payload: number[]): number[] {
   ...
   */
  generateNiimbotProductLabel(dto: ProductThermalLabelDTO): LabelPrintResponseDTO {
    ...
    const packetHexList = [startJob, setDensity, pageStart, endPage, endJob].map(pkt => 
      pkt.map(b => '0x' + b.toString(16).padStart(2, '0').toUpperCase()).join(' ')
    );
    ...
    return {
      protocol: PrinterProtocol.NIIMBOT_PRO,
   ...
   */
  generateNiimbotShippingLabel(dto: ShippingLogisticsLabelDTO): LabelPrintResponseDTO {
