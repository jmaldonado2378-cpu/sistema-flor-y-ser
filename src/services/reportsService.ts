...
import {
  TicketPromedioReport,
  StarProductsByDietReport,
  RepurchaseRateReport,
  InactiveCustomersReport,
  DailyCashAuditReport,
  CloseCashShiftDTO,
  GlobalCurrentAccountsReport,
  ExecutiveKpiSummary,
  ChannelTicketStat,
  CustomerTicketStat,
  StarProductItem,
  InactiveCustomerItem,
  CustomerAccountBalanceItem
} from '../types/reports';
...
export class ReportsService {
   ...
   */
  async getTicketPromedioReport(): Promise<TicketPromedioReport> {
    try {
      ...
      if (channelRes.rows.length > 0) {
        const totalRevenue = channelRes.rows.reduce((acc: number, row: any) => acc + parseFloat(row.total_revenue), 0);
        const totalOrdersCount = channelRes.rows.reduce((acc: number, row: any) => acc + parseInt(row.order_count), 0);
        ...
        const byChannel: ChannelTicketStat[] = channelRes.rows.map((row: any) => {
        ...
        const topCustomersByTicket: CustomerTicketStat[] = customerRes.rows.map((row: any) => ({
   ...
   */
  async getStarProductsByDiet(dietaryCode: string = 'VEGAN'): Promise<StarProductsByDietReport> {
    try {
      const query = `
        SELECT 
          p.id AS product_id,
      ...
      if (res.rows.length > 0) {
        const dietNamesMap: Record<string, { name: string; color: string }> = {
        ...
        const products: StarProductItem[] = res.rows.map((row: any, index: number) => ({
   ...
   */
  async getRepurchaseRateReport(): Promise<RepurchaseRateReport> {
    try {
      const query = `
   ...
   */
  async getInactiveCustomersReport(daysFilter?: number): Promise<InactiveCustomersReport> {
    try {
      const query = `
        SELECT 
          c.id AS customer_id,
      ...
      if (res.rows.length > 0) {
        let items: InactiveCustomerItem[] = res.rows.map((row: any) => {
          const days = row.days_inactive;
          let range: '30_DAYS' | '60_DAYS' | '90_DAYS_PLUS' = '30_DAYS';
        ...
        if (daysFilter) {
          if (daysFilter === 30) items = items.filter(i => i.daysInactive >= 30 && i.daysInactive < 60);
          else if (daysFilter === 60) items = items.filter(i => i.daysInactive >= 60 && i.daysInactive < 90);
          else if (daysFilter === 90) items = items.filter(i => i.daysInactive >= 90);
        }
   ...
   */
  async getDailyCashAudit(dateStr?: string): Promise<DailyCashAuditReport> {
   ...
   */
  async closeCashShift(dto: CloseCashShiftDTO): Promise<DailyCashAuditReport> {
   ...
   */
  async getGlobalCurrentAccountsReport(): Promise<GlobalCurrentAccountsReport> {
    try {
      const query = `
        SELECT 
          c.id AS customer_id,
      ...
      if (res.rows.length > 0) {
        const accounts: CustomerAccountBalanceItem[] = res.rows.map((row: any) => {
          const balance = parseFloat(row.current_balance);
          const limit = 50000; // Límite por defecto
          let status: 'NORMAL' | 'NEAR_LIMIT' | 'OVERDUE' | 'EXCEEDED' = 'NORMAL';

          if (balance > limit) status = 'EXCEEDED';
          else if (balance >= limit * 0.8) status = 'NEAR_LIMIT';
   ...
   */
  async getExecutiveSummary(): Promise<ExecutiveKpiSummary> {
  ...
  private getMockTicketPromedioReport(): TicketPromedioReport {
    return {
  ...
  private getMockStarProductsByDiet(dietaryCode: string): StarProductsByDietReport {
    const code = dietaryCode.toUpperCase();
    const dietMap: Record<string, { name: string; color: string; items: StarProductItem[] }> = {
      VEGAN: {
        name: 'Vegano',
        color: '#5E7055',
        items: [
  ...
  private getMockInactiveCustomers(daysFilter?: number): InactiveCustomersReport {
    const list: InactiveCustomerItem[] = [
      {
        customerId: 'inact-1',
    ...
    let filtered = list;
    if (daysFilter === 30) filtered = list.filter(c => c.inactivityRange === '30_DAYS');
    else if (daysFilter === 60) filtered = list.filter(c => c.inactivityRange === '60_DAYS');
    else if (daysFilter === 90) filtered = list.filter(c => c.inactivityRange === '90_DAYS_PLUS');
  ...
  private getMockCurrentAccountsReport(): GlobalCurrentAccountsReport {
    const accounts: CustomerAccountBalanceItem[] = [
      {
        customerId: 'cc-1',
