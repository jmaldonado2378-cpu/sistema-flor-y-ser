import { Pool } from 'pg';
import { PackagingMaterial, CreatePackagingMaterialDTO, UpdatePackagingMaterialDTO } from '../types/packaging';

const initialPackagingMaterials: PackagingMaterial[] = [
  {
    id: 'pkg-001',
    code: 'ENV-DOY-250',
...
export class PackagingService {
  private inMemoryMaterials: PackagingMaterial[] = [...initialPackagingMaterials];
  ...
  async create(dto: CreatePackagingMaterialDTO): Promise<PackagingMaterial> {
    await this.ensureTableExists();
    try {
      const res = await this.db.query(`
      ...
      const newItem: PackagingMaterial = {
        id: 'pkg-' + Date.now(),
        code: dto.code,
        name: dto.name,
  ...
  async updateStock(id: string, newStock: number): Promise<PackagingMaterial | null> {
    await this.ensureTableExists();
    try {
      const res = await this.db.query(`
  ...
  async update(id: string, dto: UpdatePackagingMaterialDTO): Promise<PackagingMaterial | null> {
    await this.ensureTableExists();
    try {
      ...
      if (dto.category !== undefined) { updates.push(`category = $${i++}`); values.push(dto.category); }
      if (dto.unit !== undefined) { updates.push(`unit = $${i++}`); values.push(dto.unit); }
      ...
      if (dto.costPerUnit !== undefined) { updates.push(`cost_per_unit = $${i++}`); values.push(dto.costPerUnit); }
      if (dto.supplierName !== undefined) { updates.push(`supplier_name = $${i++}`); values.push(dto.supplierName); }
      if (dto.storageLocation !== undefined) { updates.push(`storage_location = $${i++}`); values.push(dto.storageLocation); }
