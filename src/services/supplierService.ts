...
import {
  ...
  AccountsPayableCalendarItem,
  AccountsPayableCalendarSummary,
  ...
  ItemComprobanteInsumoGranel,
  ComprobanteInsumoGranel
} from '../types/supplier';


export class SupplierService {
  private inMemorySuppliers: Supplier[] = [];
  private inMemoryReceipts: MerchandiseReceipt[] = [];
   ...
   */
  async searchSuppliers(filters: SupplierFilterDTO = {}): Promise<{ suppliers: Supplier[]; total: number }> {
    try {
   ...
   */
  async createMerchandiseReceipt(dto: CreateMerchandiseReceiptDTO): Promise<MerchandiseReceipt> {
    ...
    try {
      ...
      try {
        ...
        const receiptRes = await client.query(receiptQuery, [
          ...
          dto.supplierId,
          dto.receiptType || ReceiptType.FACTURA,
          issueDate,
          receptionDate,
          dueDate,
          termsDays,
          ...
          if (productId) {
            await client.query(
              `UPDATE products 
               ...
               WHERE id = $3;`,
              [item.quantity, item.unitCost, productId]
            );
          } else {
            const sku = 'PROD-' + Date.now().toString().slice(-6) + Math.floor(Math.random() * 100);
            const newProdRes = await client.query(
              `INSERT INTO products (sku, name, product_type, unit_of_measure, current_stock, cost_price, supplier_id)
               VALUES ($1, $2, $3, $4, $5, $6, $7)
               RETURNING id;`,
              [sku, item.itemName, item.itemType, item.unitOfMeasure, item.quantity, item.unitCost, dto.supplierId]
            );
        ...
        return {
          ...
          receiptType: dto.receiptType || ReceiptType.FACTURA,
          issueDate,
          receptionDate,
          dueDate,
          ...
          paymentStatus: SupplierPaymentStatus.PENDING,
          items: savedItems,
          notes: dto.notes,
          createdAt,
          updatedAt: createdAt
   ...
   */
  async createRawMaterialReceipt(dto: CrearComprobanteInsumoGranelDTO): Promise<ComprobanteInsumoGranel> {
    ...
    try {
      ...
      try {
        ...
        const receiptRes = await client.query(receiptQuery, [
          ...
          dto.proveedorId,
          dto.tipoComprobante || ReceiptType.FACTURA,
          fechaEmision,
          fechaRecepcion,
        ...
        const savedItems: ItemComprobanteInsumoGranel[] = [];
        ...
        for (const item of insumosProcesados) {
          ...
          if (materiaPrimaId) {
            const rmRes = await client.query(
              'SELECT code, current_stock FROM raw_materials WHERE id = $1 FOR UPDATE',
              [materiaPrimaId]
            );
            if (rmRes.rows.length > 0) {
              ...
              await client.query(
                `UPDATE raw_materials 
                 ...
                 WHERE id = $5;`,
                [nuevoStockMateriaPrima, item.costoPorKg, dto.proveedorId, supplier.businessName, materiaPrimaId]
              );
            }
          } else if (codigoMateriaPrima) {
            const rmRes = await client.query(
              'SELECT id, current_stock FROM raw_materials WHERE code = $1 FOR UPDATE',
              [codigoMateriaPrima]
            );
            if (rmRes.rows.length > 0) {
          ...
          if (!materiaPrimaId) {
            ...
            const newRmRes = await client.query(
        ...
        const comprobante: ComprobanteInsumoGranel = {
          id: comprobanteId,
          numeroComprobante: dto.numeroComprobante,
          proveedorId: supplier.id,
          nombreProveedor: supplier.businessName,
          cuitProveedor: supplier.taxId,
          tipoComprobante: dto.tipoComprobante || ReceiptType.FACTURA,
          fechaEmision,
          fechaRecepcion,
          fechaVencimientoPago,
          diasCreditoPago,
          ...
          estadoPago: SupplierPaymentStatus.PENDING,
          items: savedItems,
          notas: dto.notas,
          creadoEn,
   ...
   */
  async crearComprobanteInsumoGranel(dto: CrearComprobanteInsumoGranelDTO): Promise<ComprobanteInsumoGranel> {
  ...
  async getMerchandiseReceiptById(receiptId: string): Promise<MerchandiseReceipt> {
    try {
      const query = `
      ...
      if (res.rows.length > 0) {
        ...
        const items = itemsRes.rows.map(i => ({
          ...
          itemType: i.item_type as MerchandiseType,
        ...
        return {
          ...
          paymentStatus: row.payment_status as SupplierPaymentStatus,
          items,
          notes: row.notes,
   ...
   */
  async searchMerchandiseReceipts(filters: ReceiptFilterDTO = {}): Promise<{ receipts: MerchandiseReceipt[]; total: number }> {
    try {
      let sql = `
        SELECT r.id
        FROM merchandise_receipts r
   ...
   */
  async registerPayment(dto: CreateSupplierPaymentDTO): Promise<SupplierPayment> {
    ...
    if (receipt.paymentStatus === SupplierPaymentStatus.PAID) {
    ...
    let newStatus: SupplierPaymentStatus = SupplierPaymentStatus.PARTIAL;
    if (newPendingBalance <= 0.01) {
      ...
      newStatus = SupplierPaymentStatus.OVERDUE;
    }
    ...
    try {
      ...
      try {
        ...
        const paymentRes = await client.query(paymentQuery, [
          ...
          dto.amount,
          paymentDate,
          dto.paymentMethod || SupplierPaymentMethod.TRANSFERENCIA,
        ...
        await client.query(
          `UPDATE merchandise_receipts 
           ...
           WHERE id = $3;`,
          [newPaidAmount, newStatus, dto.receiptId]
        );
        ...
        return {
          ...
          amount: dto.amount,
          paymentDate,
          paymentMethod: dto.paymentMethod || SupplierPaymentMethod.TRANSFERENCIA,
   ...
   */
  async getPaymentsByReceipt(receiptId: string): Promise<SupplierPayment[]> {
    try {
      const query = `
      ...
      if (res.rows.length > 0) {
        return res.rows.map(r => ({
          ...
          paymentMethod: r.payment_method as SupplierPaymentMethod,
   ...
   */
  async getAccountsPayableCalendar(filters: AccountsPayableFilterDTO = {}): Promise<{
    calendar: AccountsPayableCalendarItem[];
    summary: AccountsPayableCalendarSummary;
  }> {
    ...
    const calendar: AccountsPayableCalendarItem[] = [];
    ...
    const supplierMap: Record<string, { name: string; debt: number; count: number }> = {};

    for (const receipt of receipts) {
      if (receipt.pendingBalance <= 0.01) continue; // Ignorar totalmente abonadas
      ...
      let urgency: UrgencyLevel = 'REGULAR';
      ...
      const item: AccountsPayableCalendarItem = {
  ...
  private createSupplierInMemory(dto: CreateSupplierDTO): Supplier {
    const cleanTaxId = dto.taxId.trim().replace(/-/g, '');
  ...
  private createMerchandiseReceiptInMemory(
    dto: CreateMerchandiseReceiptDTO,
    supplier: Supplier,
    totalAmount: number,
    itemsProcessed: any[],
    issueDate: string,
    ...
    const receipt: MerchandiseReceipt = {
      id: receiptId,
      receiptNumber: dto.receiptNumber,
      supplierId: supplier.id,
      supplierName: supplier.businessName,
  ...
  private createRawMaterialReceiptInMemory(
    dto: CrearComprobanteInsumoGranelDTO,
    supplier: Supplier,
    montoTotalComprobante: number,
    insumosProcesados: any[],
    fechaEmision: string,
    ...
    const items: ItemComprobanteInsumoGranel[] = insumosProcesados.map((item, idx) => {
  ...
  private registerPaymentInMemory(
    dto: CreateSupplierPaymentDTO,
    receipt: MerchandiseReceipt,
    newPaidAmount: number,
    newStatus: SupplierPaymentStatus,
    paymentDate: string
  ): SupplierPayment {
    ...
    this.inMemoryReceipts = [
      {
        ...
        receiptType: ReceiptType.FACTURA,
        ...
        items: [
          {
            ...
            itemType: MerchandiseType.GRANEL,
            quantity: 50,
            unitOfMeasure: 'kg',
            unitCost: 7500,
            subtotal: 375000,
      ...
      },
      {
        ...
        paymentStatus: SupplierPaymentStatus.OVERDUE,
        items: [
          {
            id: 'item-102-1',
            receiptId: 'rec-102',
        ...
        items: [
          {
            ...
            itemType: MerchandiseType.ELABORADO,
            quantity: 60,
    ...
    this.inMemoryPayments = [
      {
        ...
        paymentMethod: SupplierPaymentMethod.TRANSFERENCIA,
