...
export class LabelPrintingController {
  constructor(private labelService: LabelPrintingService) {}

  printProductLabel = async (req: Request, res: Response): Promise<void> => {
    try {
      const { 
        productName, brandName, netWeightLabel, batchNumber, fractioningDate, 
        expiryDate, barcode, ingredients, dietaryBadges, copies, widthMm, heightMm 
      } = req.body;
  ...
  printShippingLabel = async (req: Request, res: Response): Promise<void> => {
    try {
      const { 
        orderNumber, customerName, customerPhoneWhatsapp, deliveryAddress, 
        neighborhoodZone, deliveryNotes, paymentStatus, totalAmount, copies, widthMm, heightMm 
      } = req.body;
