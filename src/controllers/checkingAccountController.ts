...
export class CheckingAccountController {
   ...
   */
  public getStatement = async (req: Request, res: Response): Promise<void> => {
    try {
      const { customerId } = req.params;
      ...
      if (detailed === 'true' || incluirDetalleItems === 'true') {
        const statement = await this.checkingAccountService.getExtractoDetallado({
          ...
          fechaInicio: startDate as string,
          fechaFin: endDate as string,
   ...
   */
  public registerCollection = async (req: Request, res: Response): Promise<void> => {
    try {
      const { customerId } = req.params;
      const {
        monto,
   ...
   */
  public addManualAdjustment = async (req: Request, res: Response): Promise<void> => {
    try {
      const { customerId } = req.params;
      ...
      const movement = await this.checkingAccountService.addMovement(
        customerId,
        movementType,
        amount,
        'MANUAL_ADJUSTMENT',
   ...
   */
  public updateCreditLimit = async (req: Request, res: Response): Promise<void> => {
    try {
      const { customerId } = req.params;
